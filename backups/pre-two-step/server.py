...
async def create_job(
    ...
    analytics: str = Form("true"),
):
    language = language.strip() or None
    speakers_int = int(speakers) if speakers.strip().isdigit() else None
    do_analytics = analytics.strip().lower() not in ("false", "0", "off", "no", "")
    ...
    jid = jobs.create_job(src, name, model=model, language=language, speakers=speakers_int,
                          target_language=target_language, analytics=do_analytics)
    return {"id": jid}
...
def create_v1_job(req: TranscribeRequest):
    ...
    jid = jobs.create_job(
        ...
        target_language=target, analytics=True, callback_url=req.callback_url,
    )
    return {
        "job_id": jid,
        "status": "queued",
