...
def require_api_key(key: str | None = Security(api_key_scheme)):
    # Accept the .env API_KEY (bootstrap) or any active admin-issued key. If no
    # keys are configured at all, stay open (local/dev behavior, unchanged).
    active = db.active_key_hashes()
    if not API_KEY and not active:
        return key
    if key and ((API_KEY and key == API_KEY) or admin_auth.hash_api_key(key) in active):
        return key
